...
class Condenser:
    def __init__(
        ...
        assert (
            self.maxlenL1 == self.maxlenL2
        ), "Add support for different maxlens: use two tokenizers."
    ...
    def _load_collection(self, collectionX_path):
        ...
        with open(collectionX_path) as f:
            for line_idx, line in enumerate(f):
                ...
                passage = [
                    line["title"] + " | " + sentence for sentence in line["text"]
                ]
    ...
    def _stage1(self, query, BACKS, ranking, TOPK=9):
        ...
        with torch.inference_mode():
            backs = [
                self.CollectionY[(pid, sid)]
                for pid, sid in BACKS
                if (pid, sid) in self.CollectionY
            ]
            backs = [query] + backs
            query = " # ".join(backs)
    ...
    def _stage2(self, query, preds):
        ...
        with self.amp.context():
            ...
            preds_L3x = [
                x for score, x in preds if score > min(0, preds[1][0] - 1e-10)
            ]  # Take at least 2!
            preds = [x for score, x in preds if score > 0]

            earliest_pids = f7([pid for pid, _ in preds_L3x])[
                :4
            ]  # Take at most 4 docs.
