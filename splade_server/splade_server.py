...
class SpladeServer(server_pb2_grpc.SpladeServicer):
    def __init__(self, tag):
        self.model_type_or_dir = "naver/efficient-splade-V-large-query"
        self.model = Splade(model_type_or_dir, agg="max")
        self.model.eval()
        self.tokenizer = AutoTokenizer.from_pretrained(model_type_or_dir)


    def gen_query(query_text, multiplier):
        with torch.no_grad():
            query_rep = torch.trunc(multiplier * model(q_kwargs=tokenizer(query_text, return_tensors="pt"))["q_rep"].squeeze()).int()
        col = torch.nonzero(query_rep).squeeze().cpu().tolist()
        tokens = tokenizer.convert_ids_to_tokens(col)
        counts = query_rep[col].cpu().tolist()
        query = ' '.join(word for word, count in zip(tokens, counts) for _ in range(count))
        obj = splade_pb2.QueryStr()
        obj.query = query
...
def serve_Splade_server():
    ...
    server_pb2_grpc.add_SpladeServicer_to_server(SpladeServer(), server)
    listen_addr = '[::]:50060')
